...
public class LList<T> implements List<T> { //your List.java must be in same dir
	...
	class MyIterator implements Iterator<T> { //anonymous class (hehe)(creds to mr k)
		...
		if( _curr.getNext() == null ) { return false; }
		return true;
	    }
	    
	    public T next() { //returns cargo of next DLLNode
		_curr = _curr.getNext();
		...
		//  if( okToRemove ) { //nullpointers
		//_curr.getPrev().setNext(_curr.getNext()); 
		//_curr.getNext().setPrev(_curr.getPrev());
		//_curr = _curr.getPrev();
		//}
	    }
	    //Q: how to make sure user calls next() before rm?
	}
