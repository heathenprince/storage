...
public class StatPrinter {
	...
	int max = max(data);
	ArrayList<Integer> freq = new ArrayList<Integer>(max+1);
	...
	return( i > 0 && _frequency.get(i).compareTo( _frequency.get(i-1) ) == 1 && _frequency.get(i).compareTo( _frequency.get(i+1) ) == 1 );
    }//O(1)
	...
	String retStr = "";
	int mult = longestBar/max(_frequency);
	for( int i = 0; i < _frequency.size(); i++ ) {
	    retStr += i + ":";
	    for( int c = 0; c < _frequency.get(i) * mult; c++ ) {
		retStr += "*";
	    }
	    retStr += "\n";
	}
	...
	ArrayList freddie = new ArrayList<Integer>(10);
	for( int i = 0; i < 10; i++ ) {
	    int num = (int)(Math.random() * 10 );
	    freddie.add(num);
	}

       	StatPrinter mercury = new StatPrinter(freddie);
	System.out.println(mercury.max(freddie));
       	System.out.println(mercury.isLocalMode(6));
	System.out.println(freddie);
	System.out.println(mercury._frequency);
	System.out.println(mercury.getLocalModes());
	mercury.printHistogram(10);
	}
    // */
